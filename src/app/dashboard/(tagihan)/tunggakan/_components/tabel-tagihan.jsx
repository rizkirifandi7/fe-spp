...
const StatusBadge = React.memo(({ status }) => {
	...
	switch (lowerCaseStatus) {
		...
		case "lunas":
			...
			break;
		case "partial":
			icon = <RefreshCw className={`mr-2 ${iconSize} text-yellow-600`} />;
			badgeClasses +=
...
// --- [BARU] Komponen RincianItemModal ---
const RincianItemModal = ({ isOpen, onClose, tagihan, onTagihanUpdate }) => {
	...
	const handleSendWhatsApp = async (item) => {
		...
		const jatuhTempoItem = item.jatuh_tempo
			? formatDateModal(item.jatuh_tempo)
			: "segera";
...
Tim Keuangan Sekolah
		...
		try {
			...
			toast.success(
				`Notifikasi WhatsApp untuk ${deskripsiTagihanItem} berhasil dikirim.`
			);
		} catch (error) {
	...
	const handleRenewLink = async (itemToRenew) => {
		...
		try {
			...
			const response = await fetch(
				`${apiUrl}/tagihan/item-tagihan/${itemToRenew.id}/renew-link`,
				{
					method: "POST",
			...
			toast.success(
				`Link pembayaran untuk ${itemToRenew.deskripsi} berhasil diperbarui.`
			);

			const updatedItems = tagihan.item_tagihan.map((item) =>
				item.id === itemToRenew.id
					? {
							...item,
							midtrans_url: result.data.midtrans_url,
							midtrans_order_id: result.data.midtrans_order_id,
					  }
					: item
			);

			onTagihanUpdate({ ...tagihan, item_tagihan: updatedItems });
		} catch (error) {
	...
	return (
		<div className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm flex items-center justify-center p-4 z-50">
			<Card className="bg-white w-full max-w-3xl shadow-2xl rounded-xl max-h-[90vh] flex flex-col">
				<CardHeader className="px-6 py-4 border-b bg-gray-50">
					<div className="flex justify-between items-center">
						<div>
							<CardTitle className="text-lg font-semibold text-gray-800 flex items-center">
								<ListOrdered className="h-5 w-5 mr-3 text-blue-600" /> Rincian
								Item Tagihan
							</CardTitle>
							<CardDescription className="text-sm text-blue-600 font-mono mt-1 tracking-wider">
						...
						<Table className="min-w-full">
							<TableHeader className="bg-gray-100/80">
								<TableRow className="border-b-gray-200">
									<TableHead className="py-3 px-4 text-xs font-semibold uppercase tracking-wider text-gray-500">
										Deskripsi
									</TableHead>
									<TableHead className="py-3 px-4 text-xs font-semibold uppercase tracking-wider text-gray-500">
										Jatuh Tempo
									</TableHead>
									<TableHead className="py-3 px-4 text-xs font-semibold uppercase tracking-wider text-gray-500 text-right">
										Jumlah
									</TableHead>
									<TableHead className="py-3 px-4 text-xs font-semibold uppercase tracking-wider text-gray-500 text-center">
										Status
									</TableHead>
									<TableHead className="py-3 px-4 text-xs font-semibold uppercase tracking-wider text-gray-500 text-center">
										Aksi
							...
							<TableBody className="bg-white divide-y divide-gray-200">
								{tagihan.item_tagihan.map((item) => (
									<TableRow key={item.id} className="hover:bg-blue-50/50">
										...
										<TableCell className="py-3 px-4 text-sm text-gray-800 font-medium text-right">
											{formatToIDRLocal(item.jumlah)}
										</TableCell>
										<TableCell className="py-3 px-4 text-center">
											<StatusBadge status={item.status} />
										</TableCell>
										<TableCell className="py-3 px-4 text-center space-x-1.5">
											{(item.status?.toLowerCase() === "unpaid" ||
												...
												tagihan.siswa?.telepon && (
													<Button
														...
														className="text-xs text-sky-600 border-sky-500/50 hover:bg-sky-50 hover:text-sky-700"
											...
											{(item.status?.toLowerCase() === "unpaid" ||
												...
												item.midtrans_url && (
													<Button
														...
														className="text-xs text-blue-600 border-blue-500/50 hover:bg-blue-50 hover:text-blue-700"
				...
				<CardFooter className="px-6 py-4 border-t bg-gray-50/70 flex justify-end">
					<Button
						onClick={onClose}
						className="bg-blue-600 hover:bg-blue-700 text-white font-semibold px-5 py-2 rounded-md shadow-sm"
					>
						Tutup
					</Button>
				</CardFooter>
...
// --- [DIPERBARUI] Komponen DetailTagihanModal ---
const DetailTagihanModal = ({ isOpen, onClose, tagihan }) => {
	if (!isOpen || !tagihan) return null;
	...
	return (
		<div className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm flex items-center justify-center p-4 z-50">
			<Card className="bg-white w-full max-w-2xl shadow-2xl rounded-xl max-h-[90vh] flex flex-col">
				<CardHeader className="px-6 py-4 border-b bg-gray-50">
					<div className="flex justify-between items-center">
						<div>
							<CardTitle className="text-lg font-semibold text-gray-800 flex items-center">
								<ReceiptText className="h-5 w-5 mr-3 text-emerald-600" /> Detail
								Tagihan
							</CardTitle>
				...
				<CardContent className="p-6 space-y-5 overflow-y-auto">
					<div>
						<h3 className="text-base font-semibold text-gray-700 mb-3 flex items-center"></h3>
						<div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-1 rounded-lg border border-gray-200 p-4 bg-white shadow-sm">
							...
							<ModalDetailItem
								label="NISN"
								value={tagihan.siswa?.akun_siswa?.nisn || "-"}
								icon={User}
							/>
							<ModalDetailItem
								label="Telepon Siswa"
								value={tagihan.siswa?.telepon || "-"}
								icon={Users}
							/>
							<ModalDetailItem
								label="Deskripsi Tagihan Utama"
							...
							<ModalDetailItem
								label="Total Tagihan"
								value={tagihan.total_jumlah}
								icon={DollarSign}
								isCurrency
							/>
							<ModalDetailItem
								label="Sudah Dibayar"
								value={tagihan.jumlah_bayar}
								icon={CheckCircle2}
								isCurrency
							/>
							<ModalDetailItem
								label="Sisa Tagihan"
								value={sisaTagihan}
								icon={sisaTagihan > 0 ? AlertTriangle : CheckCircle2}
								isCurrency
							/>
							<ModalDetailItem
								label="Status Tagihan Utama"
								statusValue={tagihan.status}
								icon={
									tagihan.status === "paid" || tagihan.status === "lunas"
										? CheckCircle2
										: AlertTriangle
								}
								isStatus
							...
							<ModalDetailItem
								...
								value={tagihan.jenis_pembayaran?.nama || "-"}
								icon={CreditCardIcon}
							/>
							<ModalDetailItem
								label="Tgl. Dibuat"
								value={formatDateLocal(tagihan.createdAt, true)}
								icon={CalendarDays}
							/>
						</div>
					</div>
					{tagihan.pembayaran && tagihan.pembayaran.length > 0 && (
						<div>
							<h3 className="text-base font-semibold text-gray-700 mb-3 flex items-center">
								<History className="h-5 w-5 mr-2.5 text-emerald-600" /> Riwayat
								Pembayaran ({tagihan.pembayaran.length})
							</h3>
							<div className="rounded-lg border border-gray-200 overflow-hidden shadow-sm">
								<Table>
									<TableHeader className="bg-gray-100/80">
										<TableRow className="border-b-gray-200">
											<TableHead className="py-3 px-4 text-xs font-semibold uppercase text-gray-500">
												Tanggal
											</TableHead>
											<TableHead className="py-3 px-4 text-xs font-semibold uppercase text-gray-500 text-right">
												Jumlah
											</TableHead>
											<TableHead className="py-3 px-4 text-xs font-semibold uppercase text-gray-500">
												Metode
											</TableHead>
											<TableHead className="py-3 px-4 text-xs font-semibold uppercase text-gray-500 text-center">
												Verifikasi
											</TableHead>
										</TableRow>
									</TableHeader>
									<TableBody className="bg-white divide-y divide-gray-200">
										{tagihan.pembayaran.map((p) => (
											<TableRow key={p.id} className="hover:bg-emerald-50/50">
												<TableCell className="py-3 px-4 text-sm text-gray-600">
													{formatDateTimeModal(p.createdAt)}
												</TableCell>
												<TableCell className="py-3 px-4 text-sm text-gray-800 font-medium text-right">
													{formatToIDRLocal(p.jumlah)}
												</TableCell>
												<TableCell className="py-3 px-4 text-sm text-gray-700 capitalize">
													{p.metode_pembayaran}
												</TableCell>
												<TableCell className="py-3 px-4 text-center">
													{p.sudah_verifikasi ? (
														<Badge
															variant="outline"
															className="border-emerald-500 text-emerald-600 bg-emerald-50 text-xs px-2 py-0.5 font-medium"
														>
															Terverifikasi
														</Badge>
													) : (
														<Badge
															variant="outline"
															className="border-yellow-500 text-yellow-600 bg-yellow-50 text-xs px-2 py-0.5 font-medium"
														>
															Pending
														</Badge>
													)}
				...
				<CardFooter className="px-6 py-4 border-t border-gray-200 bg-gray-50/70 flex justify-between items-center">
					<p className="text-xs text-gray-500">
						<span className="font-medium text-gray-700">{tagihan.id}</span>
					</p>
					<Button
						onClick={onClose}
						className="bg-emerald-600 hover:bg-emerald-700 text-white font-semibold px-5 py-2 rounded-md shadow-sm"
					>
						Tutup
					</Button>
				</CardFooter>
...
// --- [DIPERBARUI] Komponen TabelTagihan ---
const TabelTagihan = ({
	filteredData,
	currentPage,
	setCurrentPage,
	...
	const handleOpenRincianModal = (tagihan) => {
		setSelectedTagihan(tagihan);
										...
										<TableRow className="border-b-gray-200">
											{tableHeaders.map((headerText) => (
												<TableHead
													key={headerText}
													className={cn(
														"py-3.5 px-6 text-left text-xs font-semibold uppercase tracking-wider text-gray-500",
														headerText === "Aksi" && "text-right"
													)}
												>
													{headerText}
												</TableHead>
														...
														<div className="flex items-center">
															<div>
																<div className="text-sm font-medium text-gray-900">
																	{tagihanItem.siswa.nama}
																</div>
																<div className="text-xs text-gray-500">
																	{tagihanItem.siswa.email || ""}
																</div>
															</div>
														</div>
													</TableCell>
													<TableCell className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{`${tagihanItem.siswa.akun_siswa.kelas.nama_kelas}-${tagihanItem.siswa.akun_siswa.jurusan.nama_jurusan}`}</TableCell>
													<TableCell className="px-6 py-4 whitespace-nowrap text-sm font-mono">
														{tagihanItem.nomor_tagihan}
													</TableCell>
													<TableCell
														className="px-6 py-4 whitespace-nowrap text-sm text-gray-700 max-w-xs truncate"
														title={tagihanItem.deskripsi}
													>
														{tagihanItem.deskripsi}
													</TableCell>
													<TableCell className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800">
														{formatToIDRLocal(
															parseFloat(tagihanItem.total_jumlah) -
																parseFloat(tagihanItem.jumlah_bayar)
														)}
													</TableCell>
													<TableCell className="px-6 py-4 whitespace-nowrap">
														<StatusBadge status={tagihanItem.status} />
													</TableCell>
													<TableCell className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
														{/* Tombol Aksi Baru untuk Rincian Item */}
														<Button
															variant="outline"
															size="sm"
															className="text-blue-600 border-blue-500/50 hover:bg-blue-50 hover:text-blue-700 transition-colors duration-150"
															onClick={() =>
																handleOpenRincianModal(tagihanItem)
															}
														>
														...
														<Button
															variant="outline"
															size="sm"
															className="text-emerald-600 border-emerald-500/50 hover:bg-emerald-50 hover:text-emerald-700 transition-colors duration-150"
															onClick={() => handleOpenDetailModal(tagihanItem)}
														>
			...
			{selectedTagihan && (
				<>
					...
					<RincianItemModal
						...
						onClose={handleCloseModals}
						tagihan={selectedTagihan}
						onTagihanUpdate={handleTagihanUpdateFromModal}
					/>
				</>
			)}
		</>
